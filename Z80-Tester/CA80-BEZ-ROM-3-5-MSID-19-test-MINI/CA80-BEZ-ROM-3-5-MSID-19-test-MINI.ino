...
#define VER ".3.5.19test"  // For info
#define CR 0xd                          // ASCII CR for GH CR-LF issue
#define RC2014 0                        // 1 - RC2014, 0 - CA80mini
#define TOUCH 1                         // 1 - with touch, 0 - without
#define Z80DELAY 5                      // 2 - Z80, 5 - Z180       
//#define DEBUG
// ------------------------------------------------------------------------------
//  Stale klawiatury
// ------------------------------------------------------------------------------
...
void loop()
{
  ...
  if (!digitalRead(WAIT_))
  {
    if (!digitalRead(WR_))
    {
      ...
      {
        letter = PINA; // Read Z80 data bus D0-D7 (PA0-PA7)
        //          waitResume();
  ...
  // Check USER Key for CLK mode changes
...
void waitResume()
{
  ...
  delayMicroseconds(Z80DELAY);               // Wait 2us
  WAIT_RES_HIGH;
...
void sendDataBus(byte data)
{
  ...
  BUSREQ_LOW;                         // Request for a DMA
  WAIT_RES_LOW;                       // Now is safe reset WAIT FF (exiting from WAIT state)
  delayMicroseconds(Z80DELAY);               // Wait 2us just to be sure that Z80 read the data and go HiZ
  WAIT_RES_HIGH;
  ramEN();
  BUSREQ_HIGH;                        // Resume Z80 from DMA
}
...
void pinsSetings()
{
  ...
  pinMode(USER, OUTPUT);                          // USER led ON
  ...
  setCLK(mode);                              // FAST_CLK or SLOW_CLK
